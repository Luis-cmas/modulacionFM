...
figure(3)
plot(t,kf*Int_m)
title('Desviacion de fase de s(t)')
...
kf_hz = kf/(2*pi);
plot(t,kf_hz*m)
title('Desviacion de frecuencia de s(t)');
axis([-0.01 .11 -110 60])
grid
...
plot(t,r1);
axis([-0.04 0.15 -2.3 2.3])
title('Señal pasada por el filtro')
grid
...
hold on
plot(f,abs(M), 'b')
title('comparacion espectro de magnitud')
